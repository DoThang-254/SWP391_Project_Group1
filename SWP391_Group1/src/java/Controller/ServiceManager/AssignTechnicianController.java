...
public class AssignTechnicianController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AssignTechnicianController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AssignTechnicianController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Staff> list = td.getTechniciansByRole(1);
        request.setAttribute("requirementId", requirementId);

        request.setAttribute("list", list);
        request.getRequestDispatcher("AssignTechnician.jsp").forward(request, response);
    }
